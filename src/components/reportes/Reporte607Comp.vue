...
function exportarCSV() {
  let csv = 'RNC/Cedula,Cliente,NCF,Comprobante,Fecha,Monto,ITBIS\n'
  for (const f of facturasFiltradas.value) {
    csv += `"${f.cod_cliente || ''}","${f.nombre_cliente || ''}","${f.ncf || ''}","${f.comprobante || ''}","${f.fecha_emision || ''}","${Number(f.total) || 0}","${Number(f.impuesto) || 0}"\n`
  }
...
<template>
  <div>
    ...
    >
      ...
      <Column field="comprobante" header="Comprobante" sortable style="width: 7rem" />
      ...
      <template #empty>
        <div class="text-center py-8 text-surface-400">No hay facturas de venta en este periodo.</div>
      </template>
    </DataTable>
  </div>
</template>
