...
async function guardarImagenesTelefono(ids: string[]) {
  const imagen = serializeImageIds(ids)
  form.value.imagen = imagen
  if (!isEditing.value || !selectedTelefono.value?.id) return
  const actualizado = await window.db.update('telefonos', selectedTelefono.value.id, { imagen })
  if (!actualizado.success) throw new Error(actualizado.error || 'No se pudieron guardar las imagenes')
  selectedTelefono.value.imagen = imagen
...
async function subirImagen() {
  ...
  try {
    ...
    for (let index = 0; index < files.length; index++) {
      try {
        ids.push(await uploadImage(files[index], 'telefonos'))
        uploaded++
      } catch (error: any) {
    ...
    await guardarImagenesTelefono(ids)
    toast.add({
...
async function importarImagenInternet(imagen: any) {
  ...
  try {
    ...
    await guardarImagenesTelefono([...getImageIds(form.value.imagen), uid])
...
async function eliminarImagen(uid: string) {
  ...
  await guardarImagenesTelefono(getImageIds(form.value.imagen).filter(imageId => imageId !== uid))
}
        ...
        <div class="flex flex-col gap-2">
          ...
          <div v-if="formImages.length" class="grid grid-cols-3 gap-2">
            <div v-for="imageId in formImages" :key="imageId" class="relative aspect-square rounded-lg overflow-hidden border border-surface-200 dark:border-surface-700">
              <img :src="getImageUrl(imageId)" class="w-full h-full object-cover" alt="Imagen del telefono" />
